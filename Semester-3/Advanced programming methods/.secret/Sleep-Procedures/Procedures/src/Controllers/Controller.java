...
public class Controller {
    ...
    public void conservativeGarbageCollector(List<ProgramState> programs) {
        ...
        List<Integer> addresses = Objects.requireNonNull(
                programs.stream()
                        .map(program -> {
                            try {
                                return getAddressesFromSymbolTableAndHeap(program.getCurrentSymbolTable(), program.getHeap());
                            } catch (MyException e) {
                                throw new RuntimeException(e);
                            }
                        })
    ...
    public MyDictionaryInterface<String, Value> getSymbolTable() throws MyException {
        return this.repo.getProgramsList().get(0).getCurrentSymbolTable();
    }
    ...
    public ProcedureTableInterface getProcedures() {
        return this.repo.getProgramsList().get(0).getProceduresTable();
    }
