...
from domain.exceptions import CarException
...
class RentalService:
    ...
    def create_rental(self, rental_id, client, car, start, end):
        ...
        '''
        if self._is_car_available(rental.car, rental.start, rental.end) is False:
            raise CarException("Car is not available during that time!")

        self._repository.add(rental)
        return rental

    def _is_car_available(self, car, start, end):
        """
