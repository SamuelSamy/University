...
void testQuantity(int testingValue) {//add lots of elements
	cout << "Test quantity" << endl;
	Set s;
	
	for (int i = 10; i >= 1; i--) {
		for (int j = -testingValue; j < testingValue; j = j + i) {
			s.add(j);
		}
	}
	assert(s.size() == testingValue * 2);
	SetIterator it = s.iterator();
	...
	for (int i = 0; i < 10; i++) { //delete existing and inexistent elements
		for (int j = testingValue + 1000; j >= -testingValue - 1000; j--) {
			s.remove(j);
		}
	}
	assert(s.size() == 0);
