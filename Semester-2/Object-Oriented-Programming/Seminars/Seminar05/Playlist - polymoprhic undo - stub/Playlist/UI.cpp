...
void UI::undoAction()
{
	bool undid = this->serv.undoAction();

	if (undid)
	{
		std::cout << "Undid the last operation\n";
		return;
	}
...
void UI::redoAction()
{
	bool redid = this->serv.redoAction();

	if (redid)
	{
		std::cout << "Redid the last operation\n";
		return;
	}

	std::cout << "Nothing left to redo\n";
}
