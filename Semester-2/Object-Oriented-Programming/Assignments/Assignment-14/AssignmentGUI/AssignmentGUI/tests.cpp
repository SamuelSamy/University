...
void testAdminRepoRemove()
{
    ...
    repo0.remove(0, false);

    assert(repo0.length() == 1);

    try
    {
        repo0.remove(100, false);
        assert(false);
    }
    catch (RepositoryException error)
    {
...
void testAdminServiceUpdate()
{
    ...
    service0.update(0, 300, 400, 500, "Red", "Link1");
