...
Coat Repository::remove(int ID, bool forceRemove = false)
{
    std::vector<Coat*>::iterator it = std::find_if(this->data.begin(), this->data.end(), [ID](Coat* coat) {return coat->get_ID() == ID; });
    ...
    if ((*it)->get_quantity() != 0 && !forceRemove)
    {
...
int Repository::get_last_ID()
{
    int lastID = 0;

    for (Coat* c : this->data)
    {
        lastID = std::max(lastID, c->get_ID());
    }
