...
UserGUI::UserGUI(const UserService& _service, const AdminService& _adminService) : service(_service), adminService(_adminService)
{
    ...
    this->model = new BasketModel{ *this->service.get_repo(), *this->adminService.get_repo() };
}
...
void UserGUI::openBasketGUI()
{
    ...
    //basket.exec();
    TableViewWidget* table = new TableViewWidget{ this->model };
    table->show();
}
...
void UserGUI::openBuyingGUI()
{
    ...
    BuyingGUI*  buying = new BuyingGUI{ *this->model, this->service, this->adminService, size };
    buying->show();
}
...
void BuyingGUI::buyCoat()
{
    ...
    this->model.addCoat(coat.get_ID());
...
BuyingGUI::BuyingGUI(BasketModel& model, UserService& _service, AdminService& _adminService, int size) : model(model), service(_service), adminService(_adminService)
{
    this->index = 0;
    this->size = size;
...
TableViewWidget::TableViewWidget(BasketModel* model, QWidget* parent)
{
    ...
    table->setMinimumSize(this->size() * 2 / 3);
}
