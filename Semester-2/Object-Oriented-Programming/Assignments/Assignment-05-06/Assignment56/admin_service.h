...
class AdminService
{
    ...
    public:
        ...
        /// Constructor
        /// </summary>
        AdminService(const AdminRepository& repo) : repository(repo), lastID(0) {};
        ...
        void update(int ID, int size, int price, int quantity, std::string color, std::string photo, int changes);

        /// <summary>
        /// Gets the repo linked 
        /// </summary>
        /// <returns></returns>
        AdminRepository get_repo();
