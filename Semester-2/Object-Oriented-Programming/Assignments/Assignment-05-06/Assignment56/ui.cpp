...
void UI::printAll()
{
    ...
    std::cout << this->adminService;
}


void UI::addToBasket(int index, Vector<Coat>& coats)
{
...
{
    return userService.get_total_price(adminService);
}


void UI::showCoat(int &index, Vector<Coat> coats)
{
    index %= coats.size();
    Coat coat = coats[index];
...
void UI::handleBuying()
{
    ...
    Vector<Coat> coats = adminService.get_coats_by_size(size);
...
void UI::displayBasket()
{
    ...
    {
        Coat coat = adminService.get_coat_by_ID(repo[i].first);
        coat.set_quantity(repo[i].second);
        std::cout << coat << '\n';
    }

    std::cout << "Total price: " << userService.get_total_price(adminService) << "$\n\n";
}
