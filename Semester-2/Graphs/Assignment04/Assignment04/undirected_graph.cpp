...
UndirectedGraph::UndirectedGraph(int noOfVertices)
{
    if (noOfVertices < 0)
    {
...
bool UndirectedGraph::is_vertex(int vertex) const
{
    return this->vertices.find(vertex) != this->vertices.end();
}

bool UndirectedGraph::is_edge(int vertex0, int vertex1) const
{
...
int UndirectedGraph::count_vertices() const
{
    return (int)this->vertices.size();
}

int UndirectedGraph::count_edges() const
{
    return (int)this->costs.size();
}

int UndirectedGraph::get_degree(int vertex) const
{
...
void UndirectedGraph::set_edge_data(int vertex0, int vertex1, int value)
{
    if (!this->is_vertex(vertex0) || !this->is_vertex(vertex1))
    {
        throw std::exception(NoVertex);
...
int UndirectedGraph::get_edge_data(int vertex0, int vertex1) const
{
    if (!this->is_vertex(vertex0) || !this->is_vertex(vertex1))
    {
        throw std::exception(NoVertex);
...
void UndirectedGraph::add_edge(int vertex0, int vertex1)
{
    if (!this->is_vertex(vertex0) || !this->is_vertex(vertex1))
    {
        throw std::exception(NoVertex);
...
void UndirectedGraph::remove_edge(int vertex0, int vertex1)
{
    if (!this->is_vertex(vertex0) || !this->is_vertex(vertex1))
    {
        throw std::exception(NoVertex);
...
std::unordered_set<int>::iterator UndirectedGraph::vertices_begin() const
{
    return this->vertices.begin();
}

std::unordered_set<int>::iterator UndirectedGraph::vertices_end() const
{
    return this->vertices.end();
}

std::unordered_set<int>::iterator UndirectedGraph::edges_begin(int vertex) const
{
    return this->edges.at(vertex).begin();
}

std::unordered_set<int>::iterator UndirectedGraph::edges_out(int vertex) const
{
    return this->edges.at(vertex).end();
}

std::vector<int> UndirectedGraph::prim()
{
    struct NodeData {
        int node;
        int w;
