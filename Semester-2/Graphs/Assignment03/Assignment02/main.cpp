...
void handleOption(int option, DirectedGraph& g);
...
void printOptions()
{
    ...
    std::cout << "21. Transpose BFS\n";
    std::cout << "22. Strongly-connected components \n";
    std::cout << "23. Dijkstra\n";
    std::cout << "24. Number of distinct walks\n";
    std::cout << "25. Number of distinct walks of minimum cost\n";
    std::cout << " 0. Exit\n\n";
}
...
void handleOption(int option, DirectedGraph& g)
{
    ...
    if (option == 21)
    {
        int start = readInteger("Enter 'starting' vertex: ");
        int end = readInteger("Enter 'ending' vertex: ");

        std::vector<int> path = g.bfs_transpose(start, end);
        if (path.size() == 0)
        {
        ...
        std::string text = "Shortest path: ";
        for (const int& vertex : path)
        {
            text += std::to_string(vertex) + ", ";
        }
        text.pop_back();
        text.pop_back();
        std::cout << text;
    ...
    if (option == 22)
    {
        std::vector<std::vector<int>> components = g.strongly_connected_components();
        std::string text = "";
    ...
    if (option == 23)
    {
        ...
        std::vector<int> path = g.dijkstra(start, end, maxCost);
        if (maxCost == -1 || path.size() == 0)
        {
            std::cout << "There is no path!";
            return;
        }

        std::string text = "Shortest path has cost `" + std::to_string(maxCost) + "` and it's composed by the vertices: ";
        for (const int& vertex : path)
        {
            text += std::to_string(vertex) + ", ";
        }
    ...
    if (option == 24)
    {
        int start = readInteger("Enter starting vertex: ");
        int end = readInteger("Enter ending vertex: ");
        int pathCount = g.distinct_paths(start, end);
    ...
    if (option == 25)
    {
        int start = readInteger("Enter starting vertex: ");
        int end = readInteger("Enter ending vertex: ");
        int pathCount = g.distinct_paths_with_cost(start, end);
        std::cout << "Path count: " << pathCount;
    }
